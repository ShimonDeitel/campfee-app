import XCTest
@testable import Campfee

@MainActor
final class CampfeeTests: XCTestCase {
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(StayEntry(title: "Test", vendor: "V", amount: 10, date: Date()))
    ...
    func testCannotAddMoreAtLimitWithoutPro() {
        ...
        while store.entries.count < Store.freeLimit {
            store.add(StayEntry(title: "Filler", vendor: "V", amount: 1, date: Date()))
        }
    ...
    func testDeleteEntryRemovesIt() {
        let store = Store()
        let entry = StayEntry(title: "ToDelete", vendor: "V", amount: 5, date: Date())
        store.add(entry)
        store.delete(entry)
    ...
    func testUpdateEntryChangesFields() {
        let store = Store()
        var entry = StayEntry(title: "Original", vendor: "V", amount: 5, date: Date())
        store.add(entry)
        entry.title = "Updated"
        store.update(entry)
